...
function ext.grid.adjustgrid(by)
  ext.grid.adjustheight(by)
  ext.grid.adjustwidth(by)
end

function ext.grid.adjustheight(by)
  ext.grid.GRIDHEIGHT = math.max(1, ext.grid.GRIDHEIGHT + by)
  hydra.alert("grid is now " .. tostring(ext.grid.GRIDHEIGHT) .. " tiles tall", 1)
...
function ext.grid.resizewindow_taller()
  ext.grid.adjust_focused_window(function(f) f.h = math.min(f.h + 1, ext.grid.GRIDHEIGHT - f.y) end)
end

function ext.grid.resizewindow_shorter()
  ext.grid.adjust_focused_window(function(f) f.h = math.max(f.h - 1, 1) end)
end
